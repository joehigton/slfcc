...
library(rsample)
library(lme4)
library(MuMIn)  # For R-squared calculation
...
# ---- Function to run analysis for one seed ----
run_single_seed <- function(seed_val) {
  
  cat("Running seed:", seed_val, "\n")
  ...
  tryCatch({
    ...
    # Calculate R-squared for the demographic effects
    # Method 1: Marginal R-squared (fixed effects only)
    r_squared_marginal <- r.squaredGLMM(b_model)[1, "R2m"]
    
    # Method 2: Conditional R-squared (fixed + random effects)
    r_squared_conditional <- r.squaredGLMM(b_model)[1, "R2c"]
    
    # Extract demographic effects only (not image effects)
    # Only predict for annotators that were in the training data to avoid factor level issues
    annotators_train_pred <- annotators_train_b %>%
      mutate(b_hat = predict(b_model, newdata = annotators_train_b, re.form = NA))
    
    annotators_eval_pred <- annotators_eval %>%
      mutate(b_hat = predict(b_model, newdata = annotators_eval, re.form = NA, allow.new.levels = TRUE))
    
    # Combine predictions and scale for stable KDE
    annotators_all <- bind_rows(annotators_train_pred, annotators_eval_pred) %>%
      select(response_id, gender, race, income, educ, party, b_hat) %>%
      mutate(b_hat = as.numeric(scale(b_hat)))
    ...
    male_prop_original <- mean(is_male)
    male_prop_biased <- mean(is_male_biased)
    ...
    if(nrow(df_biased) == 0 || length(unique(df_biased$img)) < 2) {
      return(tibble(
        seed = seed_val,
        ...
        n_images = 0,
        n_annotators = 0,
        male_prop_original = male_prop_original,
        male_prop_biased = male_prop_biased,
        r_squared_marginal = NA,
    ...
    summary_metrics <- tibble(
      ...
      n_images = length(unique(df_biased$img)),
      n_annotators = length(unique(df_biased$response_id)),
      male_prop_original = male_prop_original,
      male_prop_biased = male_prop_biased,
    ...
    cat("  ERROR in seed", seed_val, ":", e$message, "\n")
    return(NULL)  # Return NULL for failed seeds
  })
}

# ---- Run analysis across seeds until we get 30 successful runs ----
cat("Starting multi-seed analysis...\n")

successful_results <- list()
seed_counter <- 1
target_successes <- 30

while(length(successful_results) < target_successes) {
  result <- run_single_seed(seed_counter)
  
  if(!is.null(result)) {
    successful_results[[length(successful_results) + 1]] <- result
    cat("  SUCCESS! (", length(successful_results), "/", target_successes, " completed)\n")
  }
  ...
  if(seed_counter > 100) {
    cat("Stopped after trying 100 seeds. Only got", length(successful_results), "successful runs.\n")
    break
  }
}

# Combine all successful results
all_results <- bind_rows(successful_results)
cat("Final sample: ", nrow(all_results)/2, " successful seeds\n")
...
all_results %>% select(method,bias,rmse,coverage) %>% 
  ...
  labs(
    x = "Method",
    y = "Value",
    title = "Distribution of Performance Metrics by Method"
  ) +
  theme_minimal() +
  theme(
...
# 1. Bias density plots
p1 <- all_results %>%
  filter(!is.na(bias)) %>%
  ggplot(aes(x = bias, fill = method, colour = method)) +
  geom_density(alpha = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "black") +
  labs(title = "Distribution of Bias Across Seeds",
       x = "Bias", y = "Density") +
  theme_minimal() +
  scale_fill_manual(values = c("Unweighted" = "red", "Reweighted" = "blue")) +
  scale_colour_manual(values = c("Unweighted" = "red", "Reweighted" = "blue"))

# 2. RMSE density plots
p2 <- all_results %>%
  filter(!is.na(rmse)) %>%
  ggplot(aes(x = rmse, fill = method, colour = method)) +
  geom_density(alpha = 0.3) +
  labs(title = "Distribution of RMSE Across Seeds",
       x = "RMSE", y = "Density") +
  theme_minimal() +
  scale_fill_manual(values = c("Unweighted" = "red", "Reweighted" = "blue")) +
  scale_colour_manual(values = c("Unweighted" = "red", "Reweighted" = "blue"))

# 3. Coverage density plots
p3 <- all_results %>%
  filter(!is.na(coverage)) %>%
  ggplot(aes(x = coverage, fill = method, colour = method)) +
  geom_density(alpha = 0.3) +
  geom_vline(xintercept = 0.95, linetype = "dashed", colour = "black") +
  labs(title = "Distribution of Coverage Across Seeds",
       x = "Coverage", y = "Density") +
  theme_minimal() +
  scale_fill_manual(values = c("Unweighted" = "red", "Reweighted" = "blue")) +
  scale_colour_manual(values = c("Unweighted" = "red", "Reweighted" = "blue"))

# 4. Bias vs seed scatter
p4 <- all_results %>%
  filter(!is.na(bias)) %>%
  ggplot(aes(x = seed, y = bias, colour = method)) +
  geom_point() +
  geom_line() +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "black") +
  labs(title = "Bias by Seed",
       x = "Seed", y = "Bias") +
  theme_minimal() +
  scale_colour_manual(values = c("Unweighted" = "red", "Reweighted" = "blue"))
...
cat("\nSample characteristics across seeds:\n")
sample_stats <- all_results %>%
  filter(method == "Unweighted") %>%  # Just need one row per seed
  summarise(
    n_images_mean = mean(n_images, na.rm = TRUE),
    n_images_range = paste(min(n_images, na.rm = TRUE), "-", max(n_images, na.rm = TRUE)),
    n_annotators_mean = mean(n_annotators, na.rm = TRUE),
    n_annotators_range = paste(min(n_annotators, na.rm = TRUE), "-", max(n_annotators, na.rm = TRUE)),
    male_bias_mean = mean(male_prop_biased - male_prop_original, na.rm = TRUE),
    male_bias_range = paste(round(min(male_prop_biased - male_prop_original, na.rm = TRUE), 3), "-", 
                            round(max(male_prop_biased - male_prop_original, na.rm = TRUE), 3))
  )
print(sample_stats)

# Count seeds that failed
failed_seeds <- all_results %>%
  filter(is.na(bias)) %>%
  pull(seed) %>%
  unique()

if(length(failed_seeds) > 0) {
  cat("\nSeeds that failed (insufficient data):", paste(failed_seeds, collapse = ", "), "\n")
} else {
